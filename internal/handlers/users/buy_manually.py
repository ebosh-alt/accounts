...
from internal.app.app import bot, client_s

from internal.filters.Filters import IsUserMessageValid
from internal.entities.database import chats, Chat, deals, accounts, sellers
...
async def start_mailing_to_seller(message: CallbackQuery, state: FSMContext):
    ...
    if chat is None:
        chat_id, er = await client_s.createChat([config.manager.seller_id], username_bot=config.telegram_bot.username, title=str(user_id))
    else:
        chat_id = chat.id
        er = False
...
async def payment(message: CallbackQuery, state: FSMContext):
    ...
    account = accs[0]
    if deal.guarantor:
        price = rounding_numbers("%.2f" % (account.price * (1 + config.shop.percent_guarantor / 100)))
    else:
        price = rounding_numbers("%.2f" % (account.price * (1 + config.shop.base_percent / 100)))
    if price < config.shop.limit_price:
