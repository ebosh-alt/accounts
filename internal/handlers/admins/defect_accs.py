...
from internal.entities.states.StateModels import TransferredMerchantAccountBalance
from service.GetMessage import get_mes
from service.keyboards import Keyboards
from internal.entities.database import deals, sellers, accounts
...
async def admin_freeze_deal(message: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(chat_id=config.manager.seller_id,
                           text=get_mes("notify_seller_freezed_deal", deal=deal),
                           )

    await bot.edit_message_text(chat_id=config.admin.main_id,
...
async def cancel_freezed_deal(message: Message, state: FSMContext):
    ...
    if transfer.status == "ACCEPTED":
        ...
        await bot.send_message(chat_id=config.admin.main_id,
